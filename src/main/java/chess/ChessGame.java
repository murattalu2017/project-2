...
public class ChessGame {
    ...
    /** The mode of the game. The default mode -- Contest. **/
    private GameMode gameMode = GameMode.Contest;
    ...
    private String getInput() {
        try {
            this.outStream.print(gameMode == GameMode.Contest ? ">" : "practice >" + " ");
            ...
            if (input == null) {
                ...
                if (input.equals("help")) {
                    ...
                    if (gameMode == GameMode.Contest) {
                      writeOutput("Undo is not supported in Contest mode.");
                    }
                    else {
                      if (!gameState.undo()) {
                  ...
                  String mode = input.replaceAll("^mode\\s+", "");
                  if (mode.equals("contest")) {
                    if (gameMode == GameMode.Practice) {
                      writeOutput("A new game will be started in contest mode.");
                      gameMode = GameMode.Contest;
                      doNewGame();
                    }
                    else {
                      writeOutput("The game is already in contest mode.");
                    }
                  }
                  else if (mode.equals("practice")) {
                    if (gameMode == GameMode.Contest) {
                      writeOutput("A new game will be started in practice mode.");
                      gameMode = GameMode.Practice;
                      doNewGame();
                    }
                    else {
                      writeOutput("The game is already in practice mode.");
                    }
                  }
                  else {
                    writeOutput("Unknown mode. Supported modes are: {contest, practice}.");
                  }
    ...
    private void showCommands() {
        ...
        writeOutput("    'undo'                       Undoes last move");
        writeOutput("    'mode contest'               Switch into contest mode");
        writeOutput("    'mode practice'              Switch into practice mode");
    }
